...
contract RegistryFacetTest is IIngesterRegistration, AccessControlFacetTest, CommonFunctionsFacetTest {
    function getIngester(address ingesterAddress)
        public
        pure
        override
        returns (IIngesterRegistration.Ingester memory)
    {
        IIngesterRegistration.Ingester memory ingester = Ingester(ingesterAddress, true, 999);
        return ingester;
    }

    function registerIngester(address ingesterAddress, string calldata message, uint256 nonce, bytes calldata sig)
        external
    {
        address controllerAddress = msg.sender;
        require(
            s.ingesterToController[ingesterAddress].controllerAddress != controllerAddress, "Ingester already exists"
        );
        ...
        s.ingesterToController[ingesterAddress] = IIngesterRegistration.IngesterToController(
            controllerAddress, s.controllerToIngesters[controllerAddress].length - 1, s.ingesterAddresses.length - 1
        );
        ++s.ingesterCount;
    ...
    function unRegisterIngester(address ingesterAddress) external onlyRegisteredController {
        ...
        require(
            s.ingesterToController[ingesterAddress].controllerAddress == controllerAddress, "Ingester does not exist"
        );
        ...
        uint256 ingesterAddressesIndexToRemove = s.ingesterToController[ingesterAddress].ingesterAddressesIndex;
        ...
        uint256 numIngestersPerController = s.controllerToIngesters[controllerAddress].length;
