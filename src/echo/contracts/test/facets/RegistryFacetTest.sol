    ...
    CommonFunctionsFacetTest
{
    ...
    ) public view override returns (IIngesterRegistration.Ingester memory) {
        IIngesterRegistration.Ingester memory ingester = Ingester(
            ingesterAddress,
            true,
            999
        );
    ...
    function registerIngester(
        ...
        uint256 clusterId = LibAppStorageTest.addIngesterToCluster(
            ingesterAddress,
            controllerAddress
        );
    ...
    function unRegisterIngester(
        ...
        if (numIngestersPerController == 1) {
            ...
            //this is a closed loop without sending eth around. IngesterProxy is fixed unless owner of contracts is taken over
            // is this still a risk? I will always have to change the ingester storage clusterId after external call
            LibAppStorageTest.removeIngesterFromGroups(
                clusterId,
                ingesterAddress
            );

            // Remove Ingester from Cluster
            LibAppStorageTest.removeIngesterFromCluster(
                ingesterAddress,
                clusterId
            );
