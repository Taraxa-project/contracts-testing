...
contract GroupManagerFacetTest is
    AccessControlFacetTest,
    CommonFunctionsFacetTest,
    IIngesterGroupManager
{
    ...
    function distributeGroup(string memory groupUsername) internal {
        ...
        require(
            s.ingesterClusters[clusterId].clusterRemainingCapacity > 0,
            "No more ingesters available to add groups to"
        );
        ...
        if (numIngesters == 1) {
            distributeGroupToIngester(
                groupUsername,
                clusterId,
                s.ingesterClusters[clusterId].ingesterAddresses[0]
            );
        } else {
            uint256 maxGroupsPerIngester = 1;
            uint256 numClusterGroups = s
                .ingesterClusters[clusterId]
                .clusterGroupCount;

            //formula to apply a balanced distribution of groups across the ingesters
            if (numClusterGroups > 0) {
                maxGroupsPerIngester = divideAndRoundUp(
                    (numClusterGroups + numIngesters - 1),
                    numIngesters
                );
            }

            for (uint256 i = 0; i < numIngesters; ++i) {
                address currentIngesterAddress = s
                    .ingesterClusters[clusterId]
                    .ingesterAddresses[i];
                if (
                    s
                        .groups[groupUsername]
                        .ingesterToGroup[currentIngesterAddress]
                        .isAdded
                ) {
                    continue;
                }

                uint256 numOfAssignedGroups = s
                    .ingesterClusters[clusterId]
                    .ingesterToAssignedGroups[currentIngesterAddress]
                    .length;
                uint256 numIngestersPerGroup = s
                    .groups[groupUsername]
                    .ingesterAddresses
                    .length;

                if (
                    numOfAssignedGroups < maxGroupsPerIngester &&
                    numIngestersPerGroup < s.maxIngestersPerGroup
                ) {
                    //assign ingester to group storage
                    s.groups[groupUsername].ingesterAddresses.push(
                        currentIngesterAddress
                    );
                    ...
                    s.groups[groupUsername].ingesterToGroup[
                        currentIngesterAddress
                    ] = IngesterToGroup(
                        true,
                        s
                    ...
                    emit IIngesterGroupManager.GroupDistributed(
                        clusterId,
                        groupUsername,
                        currentIngesterAddress
                    );
     ...
     */
    function distributeGroupsToIngester(
        address ingesterAddress
    ) external onlyRegisteredController {
        require(
            isIngesterOwnedByController(ingesterAddress, msg.sender),
            "Ingester is not owned by controller wallet"
        );
        ...
        if (numUnallocatedGroups > 0) {
            ...
            if (s.maxIngestersPerGroup > 1) {
                ...
                address ingesterToCopyGroupsFrom = s
                    .ingesterClusters[clusterId]
                    .ingesterAddresses[0];
                string[] memory groupsToAllocate = s
                    .ingesterClusters[clusterId]
                    .ingesterToAssignedGroups[ingesterToCopyGroupsFrom];
    ...
    function distributeGroupToIngester(
        string memory groupUsername,
        uint256 clusterId,
        address ingesterAddress
    ) internal {
        uint256 numOfAssignedGroups = s
        ...
        if (
            numOfAssignedGroups <= s.maxGroupsPerIngester &&
            numIngestersPerGroup < s.maxIngestersPerGroup
        ) {
            //assign ingester to group storage
    ...
    function distributeUnallocatedGroups() external {
        ...
        uint256 clusterCapacity = s
            .ingesterClusters[mostCapacityClusterId]
            .clusterRemainingCapacity;
        ...
        if (amountOfGroups > clusterCapacity) {
            ...
            while (s.unAllocatedGroups.length > allocatableAmount) {
                ...
                distributeGroupsToCluster(
                    s.unAllocatedGroups[i],
                    mostCapacityClusterId
                );
                emit IIngesterGroupManager.RemoveUnallocatedGroup(
                    s.unAllocatedGroups[i]
                );
     ...
     */
    function distributeGroupsToCluster(
        string memory groupUsername,
        uint256 clusterId
    ) internal {
        uint256 numIngesters = s
    ...
    function getMostCapacityCluster() internal view returns (uint256) {
        ...
        for (uint256 i = 0; i < s.clusterIds.length; i++) {
            if (
                s.ingesterClusters[s.clusterIds[i]].clusterRemainingCapacity >
                mostAvailableGroups
            ) {
                mostAvailableGroups = s
                    .ingesterClusters[s.clusterIds[i]]
                    .clusterRemainingCapacity;
    ...
    function removeGroup(string calldata groupUsername) external onlyAdmin {
        ...
        address[] memory ingesterAddresses = s
            .groups[groupUsername]
            .ingesterAddresses;
        removeGroupFromCluster(
    ...
    function removeGroupFromCluster(
        uint256 clusterId,
        string calldata groupUsername,
        address[] memory ingesterAddresses
    ) internal {
        ...
        for (uint256 i = 0; i < ingesterAddresses.length; i++) {
            ...
            uint256 numAssignedGroups = s
                .ingesterClusters[clusterId]
                .ingesterToAssignedGroups[currentIngesterAddress]
                .length;
            uint256 groupClusterIndex = s
                .groups[groupUsername]
                .ingesterToGroup[currentIngesterAddress]
                .groupClusterIngesterIndex;
            //Re-adjust the ingesterToAssignedGroups array
            if (groupClusterIndex != numAssignedGroups - 1) {
                string memory groupToMove = s
                    .ingesterClusters[clusterId]
                    .ingesterToAssignedGroups[currentIngesterAddress][
                        numAssignedGroups - 1
                    ];
        ...
        s.ingesterClusters[clusterId].clusterRemainingCapacity =
            (s.maxGroupsPerIngester *
                s.ingesterClusters[clusterId].ingesterAddresses.length) -
            s.ingesterClusters[clusterId].clusterGroupCount;
        emit IIngesterGroupManager.GroupRemovedFromCluster(
            clusterId,
            groupUsername
        );
    }
    ...
    function getGroup(
        string calldata groupUsername
    ) external view returns (IIngesterGroupManager.GroupSlim memory) {
        IIngesterGroupManager.GroupSlim memory group = IIngesterGroupManager
            .GroupSlim(
    ...
    function setMaxGroupsPerIngester(
        uint256 maxGroupsPerIngester
    ) external onlyAdmin {
        s.maxGroupsPerIngester = maxGroupsPerIngester;
        emit IIngesterGroupManager.MaxGroupsPerIngesterUpdated(
            maxGroupsPerIngester
        );
    }
    ...
    function setMaxIngestersPerGroup(uint256 maxIngestersPerGroup) external {
        require(
            ...
            "Can only set max ingester per group >= 1"
        );
        s.maxIngestersPerGroup = maxIngestersPerGroup;
        emit IIngesterGroupManager.MaxIngesterPerGroupUpdated(
            maxIngestersPerGroup
        );
    }

    function getGroupUsernameByIndex(
        uint256 groupIndex
